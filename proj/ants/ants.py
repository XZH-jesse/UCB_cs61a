...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if self.be_inspired == False:
            self.damage *= 2
            self.be_inspired = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        nerest_place = self.place
        while distance < self.lower_bound and not nerest_place.entrance.is_hive:
            distance += 1
            nerest_place = nerest_place.entrance
        
        while not nerest_place.entrance.is_hive and nerest_place.bees == [] and distance < self.upper_bound:
            distance += 1
            nerest_place = nerest_place.entrance

        return random_bee(nerest_place.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        list_of_place_bees = self.place.bees[:]
        super().reduce_health(amount)
        
        if self.health <= 0:
            amount += self.damage
        
        for bee in list_of_place_bees:          
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            self.turns_to_chew = (self.turns_to_chew + 1) % HungryAnt.chewing_turns
        else:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while place_behind != None:       
            if place_behind.ant != None:            
                if place_behind.ant.name != 'Queen':
                    place_behind.ant.double()
                    if place_behind.ant.is_container and place_behind.ant.ant_contained != None:
                        place_behind.ant.ant_contained.double()
                print("DEBUG", place_behind.ant.damage)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        dict = {}
        distance = 0
        front_place = self.place
        while front_place.entrance != None:
            if front_place.ant != None and front_place.ant != self:
                dict[front_place.ant] = distance
                if front_place.ant.is_container == True and front_place.ant.ant_contained != None:
                    dict[front_place.ant.ant_contained] = distance
