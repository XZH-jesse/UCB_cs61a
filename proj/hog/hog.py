...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if points == 1:
            ...
            total_points += points
        num_rolls -= 1    
    return 1 if sow_sad == True else total_points
    # END PROBLEM 1
...
def make_averaged(original_function, samples_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def call_roll_count_time(*args):
        average_time = 0
        for _ in range(samples_count):
            average_time +=  original_function(*args)
        return average_time / samples_count
    return call_roll_count_time
    # END PROBLEM 8
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if score == 0 and opponent_score < 3.5:
        return 6
    score_boar = boar_brawl(score, opponent_score)
    score_sus = sus_points(score_boar + score)
    if score_sus >= GOAL or (score_boar + score) >= GOAL:
        return 0
    if (GOAL - score) < 3.5:
        return 1
    if (GOAL - score) < 5.86:
    ...
    return sus_strategy(score, opponent_score, 11, 6)
