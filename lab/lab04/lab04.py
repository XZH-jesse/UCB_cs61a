def divide(quotients, divisors):
    ...
    """
    return {quo: [k for k in divisors if k % quo == 0] for quo in quotients}
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dia_a_to_c = distance(city_a, city_c)
    dia_b_to_c = distance(city_b, city_c)
    return get_name(city_a) if dia_a_to_c <= dia_b_to_c else get_name(city_b)
