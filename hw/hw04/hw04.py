...
def balanced(m):
    ...
    arm_left = left(m)
    arm_right = right(m)
    left_thing = end(arm_left)
    right_thing = end(arm_right)
    torque_banlanced = (length(arm_left) * total_mass(left_thing) == length(arm_right) * total_mass(right_thing))
    left_banlanced = True
    right_banlanced = True
    if is_mobile(left_thing):
        left_banlanced = balanced(left_thing)
    if is_mobile(right_thing):
        right_banlanced = balanced(right_thing)
    return left_banlanced and right_banlanced and torque_banlanced
...
def max_path_sum(t):
    ...
    for branch in branches(t):
        path_sum = max_path_sum(branch)
        if max_sub_path < path_sum:
            max_sub_path = path_sum
    return label(t) + max_sub_path
